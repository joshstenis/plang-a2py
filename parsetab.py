...
_lr_productions = [
  ...
  ('program -> stmt_list SEMICOLON','program',2,'p_program','ply-parser.py',74),
  ...
  ('stmt -> assignment','stmt',1,'p_stmt_assignment','ply-parser.py',85),
  ('stmt -> read','stmt',1,'p_stmt_read','ply-parser.py',90),
  ('stmt -> write','stmt',1,'p_stmt_write','ply-parser.py',95),
  ('stmt -> while','stmt',1,'p_stmt_while','ply-parser.py',100),
  ('stmt -> repeat','stmt',1,'p_stmt_repeat','ply-parser.py',105),
  ('stmt -> block','stmt',1,'p_stmt_block','ply-parser.py',110),
  ('stmt -> foreach','stmt',1,'p_stmt_foreach','ply-parser.py',115),
  ('stmt -> if_stmt','stmt',1,'p_stmt_if_stmt','ply-parser.py',120),
  ('block -> BEGIN stmt_list END','block',3,'p_block','ply-parser.py',125),
  ('foreach -> FOREACH ID IN LPAREN a_fact COLON a_fact RPAREN stmt','foreach',9,'p_foreach','ply-parser.py',130),
  ('while -> WHILE l_expr block','while',3,'p_while','ply-parser.py',135),
  ('repeat -> REPEAT stmt_list UNTIL l_expr','repeat',4,'p_repeat','ply-parser.py',140),
  ('if_stmt -> IF l_expr THEN stmt_list ELSE else_stmt','if_stmt',6,'p_if_stmt','ply-parser.py',145),
  ('else_stmt -> <empty>','else_stmt',0,'p_else_stmt_empty','ply-parser.py',150),
  ('else_stmt -> ELSE stmt','else_stmt',2,'p_else_stmt','ply-parser.py',155),
  ('assignment -> varref ASSIGN l_expr','assignment',3,'p_assignment','ply-parser.py',160),
  ('a_expr -> a_expr ADD a_term','a_expr',3,'p_a_expr','ply-parser.py',165),
  ('a_expr -> a_expr SUB a_term','a_expr',3,'p_a_expr','ply-parser.py',166),
  ('a_expr -> a_term','a_expr',1,'p_a_expr','ply-parser.py',167),
  ('a_term -> a_term MUL a_fact','a_term',3,'p_a_term','ply-parser.py',172),
  ('a_term -> a_term DIV a_fact','a_term',3,'p_a_term','ply-parser.py',173),
  ('a_term -> a_fact','a_term',1,'p_a_term','ply-parser.py',174),
  ('a_fact -> varref','a_fact',1,'p_a_fact','ply-parser.py',179),
  ('a_fact -> NUM','a_fact',1,'p_a_fact','ply-parser.py',180),
  ('a_fact -> LITERAL_STR','a_fact',1,'p_a_fact','ply-parser.py',181),
  ('a_fact -> SUB a_fact','a_fact',2,'p_a_fact','ply-parser.py',182),
  ('a_fact -> LPAREN a_expr RPAREN','a_fact',3,'p_a_fact','ply-parser.py',183),
  ('varref -> ID','varref',1,'p_varref','ply-parser.py',188),
  ('varref -> varref LBRACK a_expr RBRACK','varref',4,'p_varref_arr','ply-parser.py',193),
  ('l_expr -> l_expr AND l_term','l_expr',3,'p_l_expr','ply-parser.py',198),
  ('l_expr -> l_term','l_expr',1,'p_l_expr','ply-parser.py',199),
  ('l_term -> l_term OR l_fact','l_term',3,'p_l_term','ply-parser.py',204),
  ('l_term -> l_fact','l_term',1,'p_l_term','ply-parser.py',205),
  ('l_fact -> l_fact oprel a_expr','l_fact',3,'p_l_fact','ply-parser.py',210),
  ('l_fact -> a_expr','l_fact',1,'p_l_fact','ply-parser.py',211),
  ('l_fact -> LPAREN l_expr RPAREN','l_fact',3,'p_l_fact','ply-parser.py',212),
  ('oprel -> LT','oprel',1,'p_oprel','ply-parser.py',217),
  ('oprel -> GT','oprel',1,'p_oprel','ply-parser.py',218),
  ('oprel -> LEQ','oprel',1,'p_oprel','ply-parser.py',219),
  ('oprel -> GEQ','oprel',1,'p_oprel','ply-parser.py',220),
  ('oprel -> EQ','oprel',1,'p_oprel','ply-parser.py',221),
  ('oprel -> NEQ','oprel',1,'p_oprel','ply-parser.py',222),
  ('read -> READ varlist','read',2,'p_read','ply-parser.py',227),
  ('write -> WRITE expr_list','write',2,'p_write','ply-parser.py',232),
  ('varlist -> varref','varlist',1,'p_varlist','ply-parser.py',237),
  ('varlist -> varref COMMA_DELIM varlist','varlist',3,'p_varlist','ply-parser.py',238),
  ('expr_list -> a_expr','expr_list',1,'p_expr_list','ply-parser.py',243),
  ('expr_list -> expr_list COMMA_DELIM a_expr','expr_list',3,'p_expr_list','ply-parser.py',244),
]
