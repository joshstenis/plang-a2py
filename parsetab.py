...
_lr_productions = [
  ...
  ('program -> stmt_list SEMICOLON','program',2,'p_program','ply-parser.py',76),
  ('stmt_list -> stmt_list SEMICOLON stmt','stmt_list',3,'p_stmt_list','ply-parser.py',81),
  ('stmt_list -> stmt','stmt_list',1,'p_stmt_list','ply-parser.py',82),
  ('stmt -> assignment','stmt',1,'p_stmt','ply-parser.py',87),
  ('stmt -> read','stmt',1,'p_stmt','ply-parser.py',88),
  ('stmt -> write','stmt',1,'p_stmt','ply-parser.py',89),
  ('stmt -> while','stmt',1,'p_stmt','ply-parser.py',90),
  ('stmt -> repeat','stmt',1,'p_stmt','ply-parser.py',91),
  ('stmt -> block','stmt',1,'p_stmt','ply-parser.py',92),
  ('stmt -> foreach','stmt',1,'p_stmt','ply-parser.py',93),
  ('stmt -> if_stmt','stmt',1,'p_stmt','ply-parser.py',94),
  ('block -> BEGIN stmt_list END','block',3,'p_block','ply-parser.py',99),
  ('foreach -> FOREACH ID IN LPAREN a_fact COLON a_fact RPAREN stmt','foreach',9,'p_foreach','ply-parser.py',104),
  ('while -> WHILE l_expr block','while',3,'p_while','ply-parser.py',109),
  ('repeat -> REPEAT stmt_list UNTIL l_expr','repeat',4,'p_repeat','ply-parser.py',114),
  ('if_stmt -> IF l_expr THEN stmt_list ELSE else_stmt','if_stmt',6,'p_if_stmt','ply-parser.py',119),
  ('else_stmt -> <empty>','else_stmt',0,'p_else_stmt','ply-parser.py',124),
  ('else_stmt -> ELSE stmt','else_stmt',2,'p_else_stmt','ply-parser.py',125),
  ('assignment -> varref ASSIGN l_expr','assignment',3,'p_assignment','ply-parser.py',130),
  ('a_expr -> a_expr ADD a_term','a_expr',3,'p_a_expr','ply-parser.py',135),
  ('a_expr -> a_expr SUB a_term','a_expr',3,'p_a_expr','ply-parser.py',136),
  ('a_expr -> a_term','a_expr',1,'p_a_expr','ply-parser.py',137),
  ('a_term -> a_term MUL a_fact','a_term',3,'p_a_term','ply-parser.py',142),
  ('a_term -> a_term DIV a_fact','a_term',3,'p_a_term','ply-parser.py',143),
  ('a_term -> a_fact','a_term',1,'p_a_term','ply-parser.py',144),
  ('a_fact -> varref','a_fact',1,'p_a_fact','ply-parser.py',149),
  ('a_fact -> NUM','a_fact',1,'p_a_fact','ply-parser.py',150),
  ('a_fact -> LITERAL_STR','a_fact',1,'p_a_fact','ply-parser.py',151),
  ('a_fact -> SUB a_fact','a_fact',2,'p_a_fact','ply-parser.py',152),
  ('a_fact -> LPAREN a_expr RPAREN','a_fact',3,'p_a_fact','ply-parser.py',153),
  ('varref -> ID','varref',1,'p_varref','ply-parser.py',158),
  ('varref -> varref LBRACK a_expr RBRACK','varref',4,'p_varref','ply-parser.py',159),
  ('l_expr -> l_expr AND l_term','l_expr',3,'p_l_expr','ply-parser.py',164),
  ('l_expr -> l_term','l_expr',1,'p_l_expr','ply-parser.py',165),
  ('l_term -> l_term OR l_fact','l_term',3,'p_l_term','ply-parser.py',170),
  ('l_term -> l_fact','l_term',1,'p_l_term','ply-parser.py',171),
  ('l_fact -> l_fact oprel a_expr','l_fact',3,'p_l_fact','ply-parser.py',176),
  ('l_fact -> a_expr','l_fact',1,'p_l_fact','ply-parser.py',177),
  ('l_fact -> LPAREN l_expr RPAREN','l_fact',3,'p_l_fact','ply-parser.py',178),
  ('oprel -> LT','oprel',1,'p_oprel','ply-parser.py',183),
  ('oprel -> GT','oprel',1,'p_oprel','ply-parser.py',184),
  ('oprel -> LEQ','oprel',1,'p_oprel','ply-parser.py',185),
  ('oprel -> GEQ','oprel',1,'p_oprel','ply-parser.py',186),
  ('oprel -> EQ','oprel',1,'p_oprel','ply-parser.py',187),
  ('oprel -> NEQ','oprel',1,'p_oprel','ply-parser.py',188),
  ('read -> READ varlist','read',2,'p_read','ply-parser.py',193),
  ('write -> WRITE expr_list','write',2,'p_write','ply-parser.py',198),
  ('varlist -> varref','varlist',1,'p_varlist','ply-parser.py',203),
  ('varlist -> varref COMMA_DELIM varlist','varlist',3,'p_varlist','ply-parser.py',204),
  ('expr_list -> a_expr','expr_list',1,'p_expr_list','ply-parser.py',209),
  ('expr_list -> expr_list COMMA_DELIM a_expr','expr_list',3,'p_expr_list','ply-parser.py',210),
]
