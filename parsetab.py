...
_lr_productions = [
  ...
  ('block -> BEGIN stmt_list END','block',3,'p_block','ply-parser.py',92),
  ('foreach -> FOREACH ID IN LPAREN a_fact COLON a_fact RPAREN stmt','foreach',9,'p_foreach','ply-parser.py',95),
  ('while -> WHILE l_expr block','while',3,'p_while','ply-parser.py',98),
  ('repeat -> REPEAT stmt_list UNTIL l_expr','repeat',4,'p_repeat','ply-parser.py',101),
  ('if_stmt -> IF l_expr THEN stmt_list ELSE else_stmt','if_stmt',6,'p_if_stmt','ply-parser.py',104),
  ('else_stmt -> <empty>','else_stmt',0,'p_else_stmt','ply-parser.py',107),
  ('else_stmt -> ELSE stmt','else_stmt',2,'p_else_stmt','ply-parser.py',108),
  ('assignment -> varref ASSIGN l_expr','assignment',3,'p_assignment','ply-parser.py',111),
  ('a_expr -> a_expr ADD a_term','a_expr',3,'p_a_expr','ply-parser.py',114),
  ('a_expr -> a_expr SUB a_term','a_expr',3,'p_a_expr','ply-parser.py',115),
  ('a_expr -> a_term','a_expr',1,'p_a_expr','ply-parser.py',116),
  ('a_term -> a_term MUL a_fact','a_term',3,'p_a_term','ply-parser.py',119),
  ('a_term -> a_term DIV a_fact','a_term',3,'p_a_term','ply-parser.py',120),
  ('a_term -> a_fact','a_term',1,'p_a_term','ply-parser.py',121),
  ('a_fact -> varref','a_fact',1,'p_a_fact','ply-parser.py',124),
  ('a_fact -> NUM','a_fact',1,'p_a_fact','ply-parser.py',125),
  ('a_fact -> LITERAL_STR','a_fact',1,'p_a_fact','ply-parser.py',126),
  ('a_fact -> SUB a_fact','a_fact',2,'p_a_fact','ply-parser.py',127),
  ('a_fact -> LPAREN a_expr RPAREN','a_fact',3,'p_a_fact','ply-parser.py',128),
  ('varref -> ID','varref',1,'p_varref','ply-parser.py',131),
  ('varref -> varref LBRACK a_expr RBRACK','varref',4,'p_varref','ply-parser.py',132),
  ('l_expr -> l_expr AND l_term','l_expr',3,'p_l_expr','ply-parser.py',135),
  ('l_expr -> l_term','l_expr',1,'p_l_expr','ply-parser.py',136),
  ('l_term -> l_term OR l_fact','l_term',3,'p_l_term','ply-parser.py',139),
  ('l_term -> l_fact','l_term',1,'p_l_term','ply-parser.py',140),
  ('l_fact -> l_fact oprel a_expr','l_fact',3,'p_l_fact','ply-parser.py',143),
  ('l_fact -> a_expr','l_fact',1,'p_l_fact','ply-parser.py',144),
  ('l_fact -> LPAREN l_expr RPAREN','l_fact',3,'p_l_fact','ply-parser.py',145),
  ('oprel -> LT','oprel',1,'p_oprel','ply-parser.py',148),
  ('oprel -> GT','oprel',1,'p_oprel','ply-parser.py',149),
  ('oprel -> LEQ','oprel',1,'p_oprel','ply-parser.py',150),
  ('oprel -> GEQ','oprel',1,'p_oprel','ply-parser.py',151),
  ('oprel -> EQ','oprel',1,'p_oprel','ply-parser.py',152),
  ('oprel -> NEQ','oprel',1,'p_oprel','ply-parser.py',153),
  ('read -> READ varlist','read',2,'p_read','ply-parser.py',156),
  ('write -> WRITE expr_list','write',2,'p_write','ply-parser.py',159),
